...
<div class="container">
	...
	if (request.getAttribute("agent") != null) {
		Agents agent = (Agents) request.getAttribute("agent");
		boolean isOffline = "Offline".equals(agent.getStatus());
	%>
	<div class="side-section">
		<div class="agent-header">
			<div class="right-end">
				<p>
					<i class="bx bxs-star"></i><%=agent != null ? String.format("%.1f", agent.getAverageRatings()) : "N/A"%></p>
			</div>
			<div class="header-top">

				<img src="AgentProfile/<%=agent.getProfileURL()%>"
					alt="<%=agent.getProfileURL()%>" width="100" height="100"
					class="<%=isOffline ? "grayscale" : ""%>"
					onerror="this.onerror=null; this.src='RestaurantImages/default.png';" />
			</div>
			<form action="AgentStatusUpdate" method="POST">
				<div class="checkbox-wrapper-5 right">
					<div class="check">
						<input type="hidden" name="agentID"
							value="<%=agent.getAgentID()%>"> <input id="check-5"
							name="status" type="checkbox" onchange="this.form.submit();"
							value="Online"
							<%=agent.getStatus().equals("Online") ? "checked" : ""%>>
			...
			if (isOffline) {
			%>
			<div class="ribbon-2">Offline</div>
			<%
			} else {
			%>
			<div class="ribbon-2 green">Online</div>
			<%
			}
			%>
		</div>
	...
	<div class="column-sections">
		...
		<div class="deliver-order-main-container">
			...
			int liveOrderCount = 0; // Counter to track the number of live orders
			...
			if (assignedDeliveries != null && !assignedDeliveries.isEmpty()) {
				...
				for (DeliveryInfo delivery : assignedDeliveries) {
					if (liveOrderCount >= 9)
				break; // Stop processing if 9 live orders have been reached

					boolean isDelivered = "Delivered".equals(delivery.getStatus());
					if (!isDelivered) {
				foundLiveOrders = true;
				liveOrderCount++; // Increment the live order count
			...
			<div class="deliver-order-main  <%=isDelivered ? "grayscale" : ""%>">
				...
				<div class="deliver-order-header-top">
					<img
						src="RestaurantImages/<%=(restaurant != null && restaurant.getImageURL() != null) ? restaurant.getImageURL()
					...
					<div class="delivered-details">
						...
						<form action="DeliveredStatus" method="POST">
							<input type="hidden" name="orderID"
								...
								name="deliveryID" value="<%=delivery.getDeliveryID()%>">
							<input type="hidden" name="Status">

							<div class="right mt-5">
								<button type="submit" name="delivered" class="button-25">Delivered</button>
							</div>
						</form>
					</div>
					<%
